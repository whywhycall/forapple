...
# iPad 10th gen sample preset (separate app)
DEFAULT_X = 300
DEFAULT_Y = 800
...
def show_device_info():
    if not update_device_status():
        messagebox.showwarning("연결 오류", "연결된 iPad가 없습니다.")
        return
...
def swipe_test():
    if not update_device_status():
        return
    swipe(current_udid, 300, 1000, 300, 400, 0.3)
...
root = tk.Tk()
root.title("iPad 10 Auto Clicker")
root.geometry("450x520")
root.resizable(False, False)

tk.Label(root, text="iPad 10세대 자동 클릭기", font=("맑은 고딕", 14, "bold")).pack(pady=10)
